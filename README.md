...
| 素材 | 使用方式 |
|------|----------|
| PDF | 给文件路径，Agent 直接读取 |
| Markdown / 文本 / HTML | 给文件路径 |
| 纯文本 | 直接粘贴内容 |
| 图片 / 截图 | 给文件路径，Agent 读取画面、提取文字、描述图表 |

### 外挂适配器（给链接，自动抓取）

> 需要先运行 `bash skill/scripts/install.sh` 安装适配器

| 素材 | 来源 |
|------|------|
| 网页文章 | 任意 URL，自动转 Markdown |
| 微信公众号 | `mp.weixin.qq.com` 链接 |
| X / Twitter | `x.com` / `twitter.com` 链接 |
| 知乎 | `zhihu.com` 链接 |
| YouTube | 视频链接，自动提取字幕文本 |

外挂适配器提取失败时会提示你手动粘贴，不会阻塞流程。

### 手动入口（反爬严格，需人工复制）

| 素材 | 使用方式 |
|------|----------|
| 小红书 | 从 App 或网页复制内容后粘贴 |
